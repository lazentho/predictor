...
(() => {
  ...
  style.textContent = `
    ...
    #coolui.key-locked .cx-nav:not([data-panel="cx-renew"]):not(.cx-admin-only) { opacity: .25; pointer-events: none; }
    #coolui.key-locked #cx-side .cx-section-label::after { content: " LOCKED"; color: var(--cx-accent); }
    ...
    #cx-side {
      ...
      overflow-y: auto; overflow-x: hidden; scrollbar-width: thin; scrollbar-color: var(--cx-border) transparent;
    }
    #cx-side::-webkit-scrollbar { width: 6px; }
    #cx-side::-webkit-scrollbar-thumb { background: var(--cx-border); border-radius: 999px; }
    #cx-side::-webkit-scrollbar-track { background: transparent; }
    ...
    .cx-custom-expiry { display: none; grid-template-columns: repeat(3, 1fr); gap: 10px; margin: 10px 0 4px; padding: 12px; border: 1px solid var(--cx-border); border-radius: 10px; background: rgba(0,0,0,.12); }
    .cx-custom-expiry.open { display: grid; }
    .cx-custom-expiry label { display: grid; gap: 6px; font-size: 11px; font-weight: 900; opacity: .85; }
    .cx-custom-expiry .cx-input { width: 100%; text-align: center; }
    .cx-renew-card { text-align: center; padding: 24px; }
    .cx-renew-title { font-size: 28px; font-weight: 900; color: var(--cx-accent); text-shadow: 0 0 20px var(--cx-glow); margin: 4px 0 8px; }
    .cx-renew-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 18px; }
    .cx-event-line { padding: 11px 0; border-bottom: 1px solid var(--cx-border); display: grid; gap: 4px; }
    .cx-event-line:last-child { border-bottom: 0; }
    .cx-event-top { display: flex; align-items: center; justify-content: space-between; gap: 10px; font-weight: 900; }
    .cx-event-meta { font-size: 12px; opacity: .65; overflow-wrap: anywhere; }
    .cx-extend-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 10px; }
    .cx-extend-grid label { display: grid; gap: 5px; font-size: 10px; font-weight: 900; opacity: .78; text-transform: uppercase; letter-spacing: .8px; }
    .cx-extend-grid .cx-input { width: 100%; text-align: center; padding: 7px 8px; }
    .cx-extend-btn { margin-top: 8px; padding: 9px 12px; }
    ...
    <div id="coolui">
      ...
      <div id="cx-body">
        <div id="cx-side">
          ...
          <div class="cx-nav" data-panel="cx-renew" id="cx-renew-nav" style="display:none;"><div class="cx-nav-icon"><svg viewBox="0 0 24 24"><path d="M4 12a8 8 0 0 1 14-5"/><path d="M18 3v4h-4"/><path d="M20 12a8 8 0 0 1-14 5"/><path d="M6 21v-4h4"/></svg></div>Renew Key</div>
          ...
          <div class="cx-nav cx-admin-only" data-panel="cx-requests"><div class="cx-nav-icon"><svg viewBox="0 0 24 24"><path d="M21 15a4 4 0 0 1-4 4H7l-4 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"/><path d="M8 9h8"/><path d="M8 13h5"/></svg></div>Requests</div>
        ...
        <div id="cx-main">
          ...
          <!-- Renew Key -->
          <div id="cx-renew" class="cx-panel">
            <div class="cx-grid">
              <div class="cx-card cx-wide cx-renew-card">
                <p>Access Locked</p>
                <div class="cx-renew-title">Key expired</div>
                <div class="cx-small">Your Crypted UI access is locked until an admin extends your key or gives you a new one.</div>
                <div class="cx-renew-actions">
                  <button class="cx-btn" id="cx-renew-yes">Request Extension</button>
                  <button class="cx-btn cx-btn-ghost" id="cx-renew-no">Not Now</button>
                </div>
                <div id="cx-renew-message" class="cx-small" style="margin-top:12px;"></div>
              </div>
            </div>
          </div>
          ...
          <!-- Requests -->
          <div id="cx-requests" class="cx-panel">
            <div class="cx-grid">
              <div class="cx-card cx-wide">
                <p>Key Extension Requests</p>
                <div class="cx-row"><label>Admin Secret</label><input class="cx-input" id="cx-requests-secret" type="password" placeholder="admin secret" style="width:260px;"></div>
                <button class="cx-btn cx-btn-ghost" id="cx-refresh-requests">Refresh Requests</button>
                <div id="cx-request-list" class="cx-small" style="margin-top:10px;">Admin only.</div>
              </div>
            </div>
          </div>
          ...
          <div id="cx-keygen" class="cx-panel">
            <div class="cx-grid">
              <div class="cx-card cx-wide">
                ...
                <div class="cx-row"><label>Expires</label><select class="cx-select" id="cx-keygen-expiry"><option value="never">Never</option><option value="1d">1 Day</option><option value="7d">7 Days</option><option value="30d">30 Days</option><option value="90d">90 Days</option><option value="custom">Custom</option></select></div>
                <div class="cx-custom-expiry" id="cx-custom-expiry-box">
                  <label>Days<input class="cx-input" id="cx-keygen-days" type="number" min="0" max="365" value="0"></label>
                  <label>Hours<input class="cx-input" id="cx-keygen-hours" type="number" min="0" max="23" value="1"></label>
                  <label>Minutes<input class="cx-input" id="cx-keygen-minutes" type="number" min="0" max="59" value="0"></label>
                </div>
  ...
  function showMain(user) {
    ...
    loadTheme();
    setLogo(currentLogo);
    setKeyLocked(Boolean(currentUser.key_expired));
  }
  ...
  function doLogout() {
    ...
    ui.classList.remove('key-locked');
  ...
  function activatePanel(panelId) {
    document.querySelectorAll('.cx-nav').forEach(n => n.classList.toggle('active', n.dataset.panel === panelId));
    document.querySelectorAll('.cx-panel').forEach(p => p.classList.toggle('active', p.id === panelId));
  }

  function setKeyLocked(locked) {
    ui.classList.toggle('key-locked', locked);
    const renewNav = document.getElementById('cx-renew-nav');
    if (renewNav) renewNav.style.display = locked ? 'flex' : 'none';
    if (locked) {
      clearGameVisuals();
      activeVisualHelper = '';
      resetVisualState();
      activatePanel('cx-renew');
      const msg = document.getElementById('cx-renew-message');
      if (msg && !msg.textContent) msg.textContent = 'Choose whether to ask Admin for more time.';
    }
  }
  ...
  async function checkCurrentUserBanStatus() {
    ...
    try {
      ...
      if (user.key_expired) {
        setKeyLocked(true);
        document.getElementById('cx-renew-message').textContent = 'Your key expired. Request an extension to unlock the UI.';
      } else if (ui.classList.contains('key-locked')) {
        setKeyLocked(false);
        activatePanel('cx-home');
        loadCurrentKey();
        toast('Key active. UI unlocked.');
      }
    } catch (error) {
  ...
  function showAdminUserInfo(user) {
    ...
    const keyExpiry = key ? formatKeyTimeLeft(key.expires_at, 'Lifetime') : 'None';
  ...
  function formatKeyTimeLeft(expiresAt, emptyLabel = 'Lifetime', expiredLabel = 'Expired') {
    if (!expiresAt) return emptyLabel;
  ...
  async function loadCurrentKey() {
    ...
    try {
      ...
      if (expiryEl) {
        expiryEl.textContent = !key ? 'None' : formatKeyTimeLeft(key.expires_at, 'Lifetime', '0h left');
      }
    } catch {
  ...
  async function loadGeneratedKeys() {
    ...
    try {
      ...
      list.innerHTML = keys.map(k => {
        ...
        const expiry = formatKeyTimeLeft(k.expires_at, 'Lifetime');
  ...
  async function generateKeys() {
    ...
    const customExpiry = {
      days: clampNumber(document.getElementById('cx-keygen-days')?.value, 0, 365, 0),
      hours: clampNumber(document.getElementById('cx-keygen-hours')?.value, 0, 23, 0),
      minutes: clampNumber(document.getElementById('cx-keygen-minutes')?.value, 0, 59, 0)
    };
    const output = document.getElementById('cx-keygen-output');
    if (!secret) { output.textContent = 'Enter admin secret first.'; return; }
    if (expiresIn === 'custom' && customExpiry.days + customExpiry.hours + customExpiry.minutes <= 0) {
      output.textContent = 'Custom expiry must be at least 1 minute.';
      return;
    }
    adminToken = secret;
    try {
      const { keys } = await apiRequest('/api/admin/keys', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ count, expiresIn, customExpiry, createdBy: currentUser?.username || 'Admin' })
      });
  ...
  document.getElementById('cx-keygen-expiry')?.addEventListener('change', e => {
    document.getElementById('cx-custom-expiry-box')?.classList.toggle('open', e.target.value === 'custom');
  });
  document.getElementById('cx-renew-yes')?.addEventListener('click', async () => {
    const msg = document.getElementById('cx-renew-message');
    if (!currentUser?.id) return;
    try {
      await apiRequest(`/api/users/${encodeURIComponent(currentUser.id)}/renewal-request`, { method: 'POST' });
      if (msg) msg.textContent = 'Request sent to Admin. Wait for a new or extended key.';
      toast('Extension request sent.');
    } catch (error) {
      if (msg) msg.textContent = error.message;
    }
  });
  document.getElementById('cx-renew-no')?.addEventListener('click', () => {
    const msg = document.getElementById('cx-renew-message');
    if (msg) msg.textContent = 'No request sent. Your UI stays locked until your key is renewed.';
  });

  async function loadRenewalRequests() {
    const list = document.getElementById('cx-request-list');
    const secretInput = document.getElementById('cx-requests-secret');
    const secret = secretInput?.value.trim() || adminToken;
    if (!list) return;
    if (!secret) { list.textContent = 'Enter admin secret first.'; return; }
    adminToken = secret;
    try {
      const { events } = await apiRequest('/api/admin/events', { headers: authHeaders() });
      const requests = events.filter(e => e.event_type === 'key_renewal_request');
      if (!requests.length) {
        list.textContent = 'No key extension requests yet.';
        return;
      }
      list.innerHTML = requests.map(event => {
        const details = event.details || {};
        const username = details.username || event.user_id || 'Unknown user';
        const key = details.key_code || 'No key found';
        const expiry = formatKeyTimeLeft(details.expires_at, 'Lifetime');
        const created = event.created_at ? formatDate(new Date(event.created_at)) : 'Unknown';
        const userId = event.user_id || '';
        return `
          <div class="cx-event-line" data-user-id="${escapeHtml(userId)}">
            <div class="cx-event-top"><span>${escapeHtml(username)}</span><span class="cx-badge">${escapeHtml(created)}</span></div>
            <div class="cx-event-meta">Requested extension | ${escapeHtml(expiry)}</div>
            <div class="cx-event-meta">Key: ${escapeHtml(key)}</div>
            <div class="cx-extend-grid">
              <label>Days<input class="cx-input cx-extend-days" type="number" min="0" max="365" value="0"></label>
              <label>Hours<input class="cx-input cx-extend-hours" type="number" min="0" max="23" value="1"></label>
              <label>Minutes<input class="cx-input cx-extend-minutes" type="number" min="0" max="59" value="0"></label>
            </div>
            <button class="cx-btn cx-btn-ghost cx-extend-btn" data-action="extend-key">Extend This Key</button>
            <div class="cx-event-meta cx-extend-status"></div>
          </div>
        `;
      }).join('');
      list.querySelectorAll('[data-action="extend-key"]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const row = btn.closest('.cx-event-line');
          const status = row.querySelector('.cx-extend-status');
          const userId = row.dataset.userId;
          const customExpiry = {
            days: clampNumber(row.querySelector('.cx-extend-days')?.value, 0, 365, 0),
            hours: clampNumber(row.querySelector('.cx-extend-hours')?.value, 0, 23, 0),
            minutes: clampNumber(row.querySelector('.cx-extend-minutes')?.value, 0, 59, 0)
          };
          if (!userId) {
            status.textContent = 'Missing user id for this request.';
            return;
          }
          if (customExpiry.days + customExpiry.hours + customExpiry.minutes <= 0) {
            status.textContent = 'Pick at least 1 minute.';
            return;
          }
          try {
            btn.disabled = true;
            status.textContent = 'Extending key...';
            const { key } = await apiRequest(`/api/admin/users/${encodeURIComponent(userId)}/extend-key`, {
              method: 'PATCH',
              headers: authHeaders(),
              body: JSON.stringify({ customExpiry })
            });
            status.textContent = `Extended. New expiry: ${formatKeyTimeLeft(key.expires_at, 'Lifetime')}`;
            toast('Key extended.');
          } catch (error) {
  ...
  document.getElementById('cx-refresh-requests')?.addEventListener('click', loadRenewalRequests);
